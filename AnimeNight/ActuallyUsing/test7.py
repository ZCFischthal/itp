...
myCanvas.pack(side=tk.TOP, fill=tk.BOTH, expand=1)
...
entryFrame = tk.Frame(secondFrame)
entryFrame.pack(side=tk.TOP, fill=tk.BOTH, expand=1)
listBoxFrame = tk.Frame(secondFrame)
listBoxFrame.pack(side=tk.TOP, fill=tk.BOTH, expand=1, padx="20", pady="50")
buttonFrame = tk.Frame(secondFrame)
buttonFrame.pack(side=tk.TOP, fill=tk.BOTH, expand=1)
...
#def updateAnime(btnValue, entryValue):
def updateAnime(btnValue):
    ...
    number = number + 4
    number = str(number)
    #print(my_entry[entryValue+1].get())
    ...
    yeet=cur.fetchall()
    print(yeet)
    #update_statement = "SELECT * FROM anime WHERE oid = ?"
    #cur.execute(update_statement, (number,))
    #yeet = cur.fetchall()
    #print(yeet)
    #print(entryValue)
    #update_statement = "UPDATE anime SET genre=?, episode=?, date=?, rating=? WHERE id=?", (my_entry[entryValue-3], my_entry[entryValue-2], my_entry[entryValue-1], my_entry[entryValue], number,)
    #cur.execute("UPDATE anime SET id=? WHERE title=?", (number_of_btns, columns[0],))
    #update_statement = "UPDATE anime SET rating=? WHERE id=?", (my_entry[entryValue], btnValue)
...
def open_new_window():
    ...
    #canvas.pack()
    #scrollbar.pack()
    #canvas.pack(side=tk.LEFT, fill=tk.Y, expand=1)
    #scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
    #canvas.grid(row=0, column=0, sticky="nsew")
    #scrollbar.grid(row=0, column=6, sticky="ns")
    
    #tk.Label(new_window, text="hiiii").grid()
    con = sqlite3.connect("animeList.db")
    cur = con.cursor()
    ...
    for i in range(len(animes)):
        ...
        for b in range(len(column)-1):
            ...
            my_entry[number_of_values].grid(row=myRow, column=myColumn)
            myColumn+=1
            number_of_values+=1
        myColumn += 1
        #btnsEntry.append(tk.Button(new_window, command=lambda:updateAnime(myColumn, number_of_values)))
        #btnsEntry.append(tk.Button(new_window, text=f"{myRow} Update", command=lambda c=myRow: updateAnime(btnsEntry[c].cget("text"))))
        #btnsEntry.append(tk.Button(content_frame, text=f"{myRow} Update", command=lambda c=myRow: updateAnime(btnsEntry[c].cget("text"))))
        btnsEntry.append(tk.Button(new_window, text=f"{myRow} Update", command=lambda c=myRow: updateAnime(btnsEntry[c].cget("text"))))
        #command=lambda c=number_of_btns: changeText2(btns[c].cget("text"))))
        btnsEntry[myRow].grid(row=myRow, column=myColumn)
        myRow+=1
        myColumn = 0
    con.close()
...
def makeListBoxes():
    ...
    for i in range(len(schema)):
        if schema[i] != "title" and schema[i] != "id":
            ...
            myLBs.append(tk.Listbox(listBoxFrame, width=13, height=5, selectmode=tk.MULTIPLE))
